...
# Kết nối tới db
conn = sqlite3.connect('2286400001_PhamQuocAN.db')
c = conn.cursor()

# # Tạo bảng nếu chưa tồn tại
# c.execute('''
#     CREATE TABLE IF NOT EXISTS 2286400001_PhamQuocAN.db (
...
def truy_van():
    ...
    conn = sqlite3.connect('2286400001_PhamQuocAN.db')
    c = conn.cursor()
    c.execute("SELECT * FROM 2286400001_PhamQuocAN")
    records = c.fetchall()
...
def cap_nhat(record_id):
    ...
    c.execute("""UPDATE 2286400001_PhamQuocAN SET
            first_name = :first,
            last_name = :last,
            idclass = :idclass,
            year = :year,
